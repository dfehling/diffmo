...
bool jhuHadronic::filter(edm::Event &iEvent, const edm::EventSetup &iSetup)
{
	...
	for ( std::vector<pat::Jet>::const_iterator jetBegin = h_Jets->begin(), jetEnd = h_Jets->end(), ijet = jetBegin; ijet != jetEnd; ++ijet )
	{
		...
		// This allows an overall correction to be applied to all jets and subjets, for now it is always 1
        ...
        ApplyJec(ijet, jec_, jecUnc_, isData_, h_genJets, jets_CORR, npv, rhoVal, scale_, smear_, angularSmear_);
